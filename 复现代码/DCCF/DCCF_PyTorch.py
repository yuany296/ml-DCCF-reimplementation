...
# from model import * # 原始模型
from model_for_ablation import * # 消融模型
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if(args.output=='dataset'):
        ...
        logfile = logging.FileHandler('log/{}.log'.format(args.output), 'a', encoding='utf-8')# 修改：输出日志，修改为指定名字
    ...
    *********************************************************
    ...
    """
    # 修改3: 使用 device 而不是写死的 .cuda()
    # _model = DCCF(config, args).to(device) # 原始代码
    _model, ablation_config = create_ablation_model(config, args, args.ablation)
    _model = _model.to(device) #用于消融实验
    
    # 打印消融配置
    print("="*60)
    print(f"消融实验类型: {args.ablation}")
